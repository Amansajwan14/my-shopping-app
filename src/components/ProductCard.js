...
export default function ProductCard({ product, onBuyNow }) {
  ...
  const categoryIcon = categoriesWithIcons[product.category] || <ShoppingBagIcon className="w-6 h-6 text-white" />;
  ...
  return (
    <div className="bg-white shadow-md p-4 rounded hover:shadow-lg hover:scale-[1.03] transition-transform duration-300">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-sm font-semibold text-gray-500">{product.category || "General"}</span>
      </div>
